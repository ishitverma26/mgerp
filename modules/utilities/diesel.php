...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create' || ($action === 'edit' && $isAdmin)) {
        ...
        if ($receivedAt === '' || !is_numeric($qty) || $qty <= 0 || ($price !== null && (!is_numeric($price) || $price < 0))) {
            ...
            $old = $pdo->prepare("SELECT * FROM diesel_log WHERE id=:id");
            $old->execute([':id' => $id]);
            $oldRow = $old->fetch();
            $pdo->prepare("UPDATE diesel_log SET received_datetime=:dt, quantity_liters=:q, price_paid=:p, remarks=:r, updated_at=NOW() WHERE id=:id")
                ->execute([':dt' => $receivedAt, ':q' => $qty, ':p' => $price, ':r' => $remarks !== '' ? $remarks : null, ':id' => $id]);
            logAudit($pdo, $currentUser['id'], 'update', 'diesel_log', $id, $oldRow, [
                'received_datetime' => $receivedAt, 'quantity_liters' => $qty, 'price_paid' => $price, 'remarks' => $remarks,
            ]);
            setFlash('success', 'Entry updated.');
        } else {
            $stmt = $pdo->prepare(
                "INSERT INTO diesel_log (received_datetime, quantity_liters, price_paid, remarks, created_by) VALUES (:dt, :q, :p, :r, :uid)"
            );
            $stmt->execute([
                ':dt' => $receivedAt, ':q' => $qty, ':p' => $price, ':r' => $remarks !== '' ? $remarks : null, ':uid' => $currentUser['id'],
            ]);
            logAudit($pdo, $currentUser['id'], 'create', 'diesel_log', (int) $pdo->lastInsertId(), null, [
                'received_datetime' => $receivedAt, 'quantity_liters' => $qty, 'price_paid' => $price, 'remarks' => $remarks,
            ]);
            setFlash('success', 'Diesel entry logged.');
        }
        redirect('/modules/utilities/diesel.php');
    }
    ...
    if ($action === 'delete' && $isAdmin) {
        ...
        $old->execute([':id' => $id]);
        $oldRow = $old->fetch();
        $pdo->prepare("DELETE FROM diesel_log WHERE id=:id")->execute([':id' => $id]);
        logAudit($pdo, $currentUser['id'], 'delete', 'diesel_log', $id, $oldRow, null);
...
if ($isAdmin && !empty($_GET['edit'])) {
    $stmt = $pdo->prepare("SELECT * FROM diesel_log WHERE id=:id");
    $stmt->execute([':id' => (int) $_GET['edit']]);
    $editRow = $stmt->fetch();
}

$rows = $pdo->query(
    "SELECT d.*, u.name AS user_name FROM diesel_log d JOIN users u ON u.id = d.created_by ORDER BY d.received_datetime DESC"
)->fetchAll();
...
<div class="activity-card-row" style="margin-bottom:16px;">
  <div class="activity-card">
    <div class="activity-card-top">
      <span class="activity-card-icon activity-card-icon-amber"><?= tabIcon('truck') ?></span>
...
<details class="card">
  <summary class="flex-between">
    <h3 class="mt-0" style="margin:0;">Log Diesel Received</h3>
  ...
  <form method="post" action="">
    ...
    <div class="form-row">
      ...
      <div><label>Quantity (Liters) *</label><input type="number" step="0.01" min="0.01" name="quantity_liters" required></div>
    </div>
    <div class="form-row">
      <?php if ($isAdmin): ?>
      <div><label>Price Paid (Rs) *</label><input type="number" step="0.01" min="0" name="price_paid" required></div>
      <?php endif; ?>
...
<details class="card"<?= $editRow ? ' open' : '' ?>>
  <summary class="flex-between">
    <h3 class="mt-0" style="margin:0;"><span class="icon-chip icon-chip-amber" style="margin-bottom:0;vertical-align:-8px;margin-right:6px;"><?= tabIcon('truck') ?></span>Diesel Log</h3>
    <span class="batch-chevron"><?= tabIcon('chevron') ?></span>
  </summary>
  <div class="card-detail">
  <?php if (!$rows): ?>
    <p class="row-card-empty">No diesel entries logged yet.</p>
  <?php else: ?>
  <div class="row-list">
      ...
      <details class="row-card row-card-info"<?= $isEditing ? ' open' : '' ?>>
        ...
        <?php if ($isEditing): ?>
          <form method="post" action="">
            ...
            <div class="row-card-actions" style="margin-top:12px;">
              ...
              <a href="<?= APP_URL ?>/modules/utilities/diesel.php<?= $embedMode ? '?embed=1' : '' ?>" class="btn btn-sm btn-outline">Cancel</a>
            </div>
          </form>
        <?php else: ?>
        ...
        <?php if ($isAdmin): ?>
          <div class="row-card-actions" style="margin-top:12px;">
            ...
            <form method="post" style="display:inline" data-confirm="Delete this diesel entry? This cannot be undone.">
